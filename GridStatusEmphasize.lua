...
-- Determine if guid is actively emphasized.
local function IsEmphasized(t, guid, tag)
    if t[guid] and t[guid][tag] then
        return true
    end
    return false
end
...
function GridStatusEmphasize:IsEmphasized(guid, tag)
    return IsEmphasized(self.temp_emphasized_units, guid, tag)
end
